...
export const useSyllabus = (rootNote: string, scaleType: ScaleType, currentPosition: number) => {
    ...
    const fetchLesson = useCallback(async (userQuery?: string, mode: 'CURRICULUM' | 'PHRASING' = 'CURRICULUM', pattern?: string) => {
        if (!currentChapter && mode === 'CURRICULUM') return;
        setLoadingLesson(true);
        const content = await getScaleLesson(rootNote, scaleType, currentChapter, currentPosition, userQuery, mode, pattern);
        setLesson(content);
