...
const App: React.FC = () => {
  ...
  const selectChapter = (index: number) => {
    ...
    setScaleType(CHAPTERS[index].targetScaleType);
    setPhase('PREVIEW');
    setChallengeComplete(false);
    setSuccessNoteIds([]);
    setActiveStepIndex(0);
            ...
            <div className="space-y-4">
              {CHAPTERS.map((ch, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold transition-all shadow-lg ${
                      isCompleted ? 'bg-green-500 text-white' : isActive ? 'bg-amber-500 text-slate-950 scale-110' : 'bg-slate-800 text-slate-600 group-hover:bg-slate-700 group-hover:text-slate-400'
                    }`}>
